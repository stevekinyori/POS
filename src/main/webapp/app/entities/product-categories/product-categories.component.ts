...
@Component({
    selector: 'jhi-product-categories',
...
export class ProductCategoriesComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.productCategoriesService.search({
    ...
    trackId(index: number, item: ProductCategories) {
        return item.id;
    }
    registerChangeInProductCategories() {
        this.eventSubscriber = this.eventManager.subscribe('productCategoriesListModification', (response) => this.reset());
    }
