...
@Component({
    selector: 'jhi-supplier-products',
...
export class SupplierProductsComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.supplierProductsService.search({
    ...
    trackId(index: number, item: SupplierProducts) {
        return item.id;
    }
    registerChangeInSupplierProducts() {
        this.eventSubscriber = this.eventManager.subscribe('supplierProductsListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.supplierProducts.push(data[i]);
        }
    }
