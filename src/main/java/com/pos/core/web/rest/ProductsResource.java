...
public class ProductsResource {
    ...
    public ResponseEntity<List<Products>> getAllProducts(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("suppliers-is-null".equals(filter)) {
            log.debug("REST request to get all Productss where suppliers is null");
            return new ResponseEntity<>(productsService.findAllWhereSuppliersIsNull(),
                    HttpStatus.OK);
        }
        if ("receivedfromsupplier-is-null".equals(filter)) {
            log.debug("REST request to get all Productss where receivedFromSupplier is null");
            return new ResponseEntity<>(productsService.findAllWhereReceivedFromSupplierIsNull(),
                    HttpStatus.OK);
        }
